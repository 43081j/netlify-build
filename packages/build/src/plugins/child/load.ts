...
export const load = async function ({ pluginPath, inputs, packageJson, verbose, netlifyConfig }) {
  ...
  const methods = Object.fromEntries(Object.entries(logic as Record<PropertyKey, unknown>).filter(isEventHandler))
...
const isEventHandler = function ([_event, value]: [unknown, unknown]): boolean {
